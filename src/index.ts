...
async function main(): Promise<void> {
    ...
    gateway.onMessage(async (message, reply) => {
        logInfo(`📩 Gateway [${message.channel}] from ${message.senderName}: "${message.text.slice(0, 80)}"`);
        try {
            const result = await orchestrator.executeTask(
    ...
    logInfo(`
╔═══════════════════════════════════════════╗
║    📱🖥️ Yakshas-v1 v5.0.0 — TIER 6     ║
║  Multi-LLM + ADB + PC + Google + Gateway  ║
║                                           ║
...
║   ✅ PC desktop control ready            ║
║   ✅ Persistent memory active            ║
║   ✅ Dynamic plugin system ready         ║
║                                           ║
║   Send a message to your Telegram bot     ║
║   to start controlling your phone & PC.   ║
║                                           ║
